...
class Settings(BaseSettings):
    ...
    # Storage backend for recorded answer audio — must match the main exam
    # backend's storage so both services read/write the same files instead
    # of each keeping its own separate copy.
    storage_backend: str = "local"
    audio_dir: str = "./storage/audio"
    # Cloudflare R2 (required when storage_backend=cloud)
    r2_endpoint_url: str = ""
